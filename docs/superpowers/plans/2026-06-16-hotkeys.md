...
export function parseShortcut(shortcut: string): ParsedShortcut {
  ...
  const cmdOrCtrl = parts.slice(0, -1).includes("CmdOrCtrl");
  const ctrl = parts.slice(0, -1).some((part) => part === "Ctrl" || part === "Control");
  if (cmdOrCtrl && ctrl) throw new Error(`invalid mixed command modifiers: ${shortcut}`);
  return {
...
export function shortcutMatches(event: KeyboardLikeEvent, shortcut: ParsedShortcut, platform: PlatformName): boolean {
  ...
  if (shortcut.cmdOrCtrl) {
    const expectedCommand = platform === "darwin" ? event.metaKey && !event.ctrlKey : event.ctrlKey && !event.metaKey;
    if (!expectedCommand) return false;
  } else if (!shortcut.ctrl && (event.metaKey || event.ctrlKey)) return false;
...
const MENU_ACTIONS: &[(&str, &str, &str, &str)] = &[
    ...
    ("Workspace", "workspace.previousTab", "Previous Tab", "Ctrl+Shift+Tab"),
