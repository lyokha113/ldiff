...
**Goal:** Hide Java compiler-generated nested/anonymous `.class` entries from the file tree when their parent `.class` exists in the same folder, and align file icons with folder/archive icons.

**Architecture:** Keep the behavior in the React adapter. `src/lib/tree.ts` projects raw `ComparePair` entries into a tree-visible model while preserving all raw archive entries for lower layers. `src/components/FileTree.tsx` renders a fixed chevron spacer in file cells so visual alignment matches folder/archive rows.
...
- Modify `src/lib/tree.test.ts`: cover parent-present suppression, orphan visibility, non-class `$` visibility, and same-folder behavior.
...
```ts
  ...
  it("does not hide non-class files that contain a dollar sign", () => {
    ...
    expect(assets.children.map((n) => n.name)).toEqual(["foo$bar.txt", "foo.txt"]);
  });
...
Update `finalize` to filter files before sorting:
